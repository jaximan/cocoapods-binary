...
module Pod
  class Prebuild
    class SharedCache
      ...
      def self.has?(target, options)
        if Podfile::DSL.shared_cache_enabled
          ...
          if not result and Podfile::DSL.shared_s3_cache_enabled
            ...
            if s3.bucket(Podfile::DSL.s3_options[:bucket]).object("#{Podfile::DSL.s3_options[:prefix]}/#{s3_cache_path}").exists?
              Dir.mktmpdir {|dir|
                s3.bucket(Podfile::DSL.s3_options[:bucket]).object("#{Podfile::DSL.s3_options[:prefix]}/#{s3_cache_path}").get(response_target: "#{dir}/framework.zip")
      ...
      #
      # @return [Pathname]
      def self.framework_cache_path_for(target, options, include_cache_root)
